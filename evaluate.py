...
def test_single_case(net, image, stride, patch_size, num_classes=1, model_type='vnet'):
    """
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                out = net(test_patch)
                # Support models that return either logits or (features, logits)
...
def decode_label(label):
    ...
    Supports both common BraTS codings:
    - Original: {0: bg, 1: NCR/NET, 2: ED, 4: ET}
    - Remapped contiguous: {0: bg, 1: NCR/NET, 2: ED, 3: ET}
    ...
    Important:
    - Keep region definitions fixed across all cases.
    - Do not infer ET id from per-case unique values, otherwise ET/TC can
      be mis-computed for cases where ET is absent.
    """
    # Accept tuple inputs like (label_map, score_map)
    if isinstance(label, (tuple, list)):
        label = label[0]
    label = np.asarray(label)

    # Fixed BraTS region definitions:
    # - ET can be encoded as 3 (contiguous remap) or 4 (original BraTS)
    # - TC = NCR/NET (1) union ET
    wt = (label != 0).astype(np.uint8)
    et = ((label == 3) | (label == 4)).astype(np.uint8)
    tc = ((label == 1) | (et == 1)).astype(np.uint8)
    return wt, tc, et
...
if __name__ == "__main__":
    ...
    f.close()
    imglist = [data_dir + "/brats2020/" + x + ".npy" for x in imglist]
