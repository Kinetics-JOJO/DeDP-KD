...
class WeightedCrossEntropyLoss(nn.Module):
    ...
    def forward(self, predict, target):
        # Align target shape if needed
        if len(target.shape) == len(predict.shape):
            assert target.shape[1] == 1
            target = target[:, 0]

        # Build robust class weights even if some classes absent in the crop
        with torch.no_grad():
            counts = []
            total = torch.tensor(0.0, device=target.device)
            for c in range(self.num_classes):
                cnt = torch.sum(target == c).float()
                counts.append(cnt)
                total += cnt
            counts = torch.stack(counts)
            total = torch.clamp(total, min=1.0)
            weight = 1.0 - counts / total
            # Normalize weights to keep scale stable
            wsum = torch.clamp(torch.sum(weight), min=1e-6)
            weight = weight * (self.num_classes / wsum)
            # Sanitize weights to avoid numerical issues
            weight = torch.nan_to_num(weight, nan=1.0, posinf=1.0, neginf=1.0)
            weight = torch.clamp(weight, min=1e-3, max=10.0)
...
def softmax_kl_loss(input_logits, target_logits, reduction: str = "batchmean"):
